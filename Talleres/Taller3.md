...
for file in $(ls)
do 
echo "##Nombre del Archivo: $file" >> archivosSRC.md
echo "\`\`\`" >> archivosSRC.md
cat $file >> archivosSRC.md
echo "\`\`\`" >> archivosSRC.md
done

```

*Nota: Recordar que la comilla deben ser precedidos por un backslash porque es un caracter especial.*
...
*Notas: 1. `cc` es para compilar los archivos. 2. Para el resto de comandos desconocidos, recurrir a los links ubicados en el enunciado.*
